...
async function playEpisode(animeTitle, episodeNumber) {
  ...
  try {
    ...
    const searchResults = await searchAnimepahe(animeTitle);
    ...
    // Step 2: Get episodes via backend
    ...
    if (!isVerified) {
      // Open Animepahe for verification
      const playUrl = `https://animepahe.pw/play/${anime.session}/${sessionId}`;
      window.open(playUrl, '_blank');
      saveVerifiedSession(anime.id, anime.session);
      
      // Show verification screen
      showVerificationScreen(anime, episode, animeTitle, actualEpNum);
      return;
    }
    
    // Step 5: Get embed and play
    const embedData = await getAnimepaheEmbed(anime.session, sessionId);
    
    if (!embedData || !embedData.success) {
      // Fallback: open play page directly
      const playUrl = `https://animepahe.pw/play/${anime.session}/${sessionId}`;
    ...
    // Step 6: Show compact player
    showCompactPlayer(embedData.embedUrl, anime.title, actualEpNum, episodes.length);
...
console.log('📼 REWIND loaded — Working episode player with backend!');
